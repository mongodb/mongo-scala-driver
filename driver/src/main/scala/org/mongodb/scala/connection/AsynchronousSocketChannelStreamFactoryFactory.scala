...
object AsynchronousSocketChannelStreamFactoryFactory {
  def apply(socketSettings: SocketSettings, sslSettings: SslSettings): StreamFactory =
    new JAsynchronousSocketChannelStreamFactoryFactory().create(socketSettings, sslSettings)
}
