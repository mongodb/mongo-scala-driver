...
case class DocumentCodecProvider() extends CodecProvider {
  ...
  def get[T](clazz: Class[T], registry: CodecRegistry): Codec[T] = {
    clazz match {
      case c if classOf[ImmutableDocument] => ImmutableDocumentCodec(registry).asInstanceOf[Codec[T]]
      case c if classOf[MutableDocument] => MutableDocumentCodec(registry).asInstanceOf[Codec[T]]
      case _ => null
    }
  }
  // scalastyle:on null
