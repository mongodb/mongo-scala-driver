...
import com.mongodb.scala.collection.immutable
...
case class MongoDatabase(private val wrapped: JMongoDatabase) {
   ...
   */
  def getCollection[T](collectionName: String)(implicit e: T DefaultsTo immutable.Document, ct: ClassTag[T]): MongoCollection[T] =
   ...
   */
  def runCommand[T](command: Bson)(implicit e: T DefaultsTo immutable.Document, ct: ClassTag[T]): Observable[T] =
   ...
   * @return a Observable containing the command result
   */
  def runCommand[T](command: Bson, readPreference: ReadPreference)(implicit e: T DefaultsTo immutable.Document, ct: ClassTag[T]): Observable[T] =
   ...
   */
  def listCollections[T]()(implicit e: T DefaultsTo immutable.Document, ct: ClassTag[T]): ListCollectionsObservable[T] =
