...
case class MongoDatabase(name: String, client: MongoClient, options: MongoDatabaseOptions) {
  ...
  def executeAsyncCommand(command: Document): Observable[CommandResult] = executeAsyncCommand(command, readPreference)
  def executeAsyncCommand(command: Document, readPreference: ReadPreference): Observable[CommandResult] = {
