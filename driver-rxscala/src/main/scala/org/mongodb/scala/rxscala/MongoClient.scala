...
object MongoClient extends GetDefaultCluster {
  ...
  def apply(seedList: List[ServerAddress], credentialList: List[MongoCredential], options: MongoClientOptions,
            bufferProvider: BufferProvider): MongoClient = {
    // scalastyle:off null
 ...
 * The MongoClient
 *
 * Normally created via the [[org.mongodb.scala.rxscala.MongoClient$ MongoClient]] companion object helpers
 *
