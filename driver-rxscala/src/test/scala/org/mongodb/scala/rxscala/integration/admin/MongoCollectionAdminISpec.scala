...
class MongoCollectionAdminISpec extends RequiresMongoDBSpec {
  ...
  it should "tell if a collection is capped or ordinary collection" in withDatabase {
    database =>
      database.admin.createCollection("collection").toBlockingObservable.first
      database("collection").admin.isCapped.toBlockingObservable.first shouldBe false
  }
  ...
  it should "tell if a collection is capped for a capped collection" in withDatabase {
    database =>
      ...
      database.admin.createCollection(collectionOptions).toBlockingObservable.first
      database("testCapped").admin.isCapped.toBlockingObservable.first shouldBe true
  }
